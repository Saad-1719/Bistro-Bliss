...
const MenuPage = () => {
  ...
  useEffect(() => {
    const getMenu = async () => {
      ...
      setError(null);
      try {
        const response = await fetch("https://bristo-bliss-backend-hosting-env.eba-z4tdcyhg.eu-north-1.elasticbeanstalk.com/meals");
        if (!response.ok) {
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10">
          ...
          {filteredMenu.map((item) => (
            ...
            >
              <img
                src={`https://bristo-bliss-backend-hosting-env.eba-z4tdcyhg.eu-north-1.elasticbeanstalk.com/${item.image}`}
                alt={item.name}
