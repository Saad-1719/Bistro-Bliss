  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case "email":
        setErrors((prev) => ({
          ...prev,
          [name]: !value.includes("@") ? "Invalid email address" : "",
        }));
        return value.includes("@");
      case "phone":
        setErrors((prev) => ({
          ...prev,
          [name]: value.length !== 11 ? "Phone number must be 11 digits long" : "",
        }));
        ...
        <div className="max-w-4xl mx-auto mt-8 sm:mt-10">
          {orderSuccess ? (
            <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-6 rounded-lg text-center">
              ...
              <p className="text-base sm:text-lg mt-2">Thank you for your order. We'll process it soon.</p>
            </div>
          ) : (
            <form
            ...
            >
              ...
              <div className="w-full">
                ...
                <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6 mt-2">
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor="cod" className="text-[#2C2F24] text-sm sm:text-base">
              ...
              <div className="w-full space-y-2">
                {touched.name && errors.name && (
                  <p className="text-[#AD343E] text-xs sm:text-sm">{errors.name}</p>
                )}
                {touched.email && errors.email && (
                  <p className="text-[#AD343E] text-xs sm:text-sm">{errors.email}</p>
                )}
                {touched.phone && errors.phone && (
                  <p className="text-[#AD343E] text-xs sm:text-sm">{errors.phone}</p>
                )}
                {orderError && (
              ...
              <button
                ...
                className={`text-center w-full rounded-full bg-[#AD343E] font-semibold text-white px-4 py-2 sm:py-3 text-sm sm:text-base hover:cursor-pointer active:scale-90 transition duration-300 ease-in-out hover:bg-[#ad343e]/90 ${
                  isSubmitting || !getFormValidity() ? "opacity-70 cursor-not-allowed" : ""
                }`}
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-6 sm:gap-4 pt-6 sm:pt-8">
            <div className="flex-1">
              <h1 className="text-lg sm:text-xl font-bold pb-3 sm:pb-4">Need Help?</h1>
            ...
            <div className="flex-1">
              <h1 className="text-lg sm:text-xl font-bold pb-3 sm:pb-4">Our Address:</h1>
